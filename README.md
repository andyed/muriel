...
- **[Steve at Druck I.T. — “I Turned My Git History Into a UX Skill”](https://druckit.wordpress.com/2026/08/19/i-turned-my-git-history-into-a-ux-skill/)** — the direct inspiration for Muriel's host-native review pass: diagnose visual symptoms before implementation syntax, carry portable design judgment across codebases without importing their local values, preserve the target product's visual language, review the surrounding composition, and ask what the user would probably correct before handoff. The article's larger correction-history idea also prompted the queued Session Cartographer judgment-mining proposal in [`TODO.md`](TODO.md). Steve is a former boss of Andy's who taught him a great deal about software development; this credit is personal as well as intellectual.
- **[anup-a/svgwave](https://github.com/anup-a/svgwave)** — the approachable layered-wave generator that prompted Muriel's `wavefield()` primitive. Muriel credits the product idea but does not copy, translate, vendor, import, or depend on its source: the repository's `LICENSE` prohibits commercial use and modification, while `package.json` labels the project ISC. Muriel instead uses an independently implemented, deterministic harmonic/normalized-series model with inspectable geometry, viewBox-first SVG, accessible metadata, and no animation by default.
...
- **[cathrynlavery/diagram-design](https://github.com/cathrynlavery/diagram-design)** (MIT, 14.5k★) — editorial diagram skill: self-contained HTML + SVG, hairline dividers, nothing in `<defs>` but the arrow marker, one accent per figure. The only entry in this list with direct code lineage into muriel — the layout proportions behind [`layer_stack`](muriel/tools/diagrams/layer_stack.py), [`pyramid`](muriel/tools/diagrams/pyramid.py), and [`swimlane`](muriel/tools/diagrams/swimlane.py) (band/tier/lane heights, the linear-taper and proportional-width rules, lane dividers and handoff emphasis, label placement, the single-focal-accent convention) are adapted from its `type-layers` / `type-pyramid` / `type-swimlane` references, 2026-05, full notice in [`THIRD_PARTY_NOTICES.md`](THIRD_PARTY_NOTICES.md). muriel took the geometry and the restraint philosophy, not the typefaces or colour system, and replaced the source's WCAG AA with the 8:1 floor. **Upstream has moved a long way since that adaptation** (re-surveyed 2026-08-13): 28 files → ~120, 14 `type-*.md` references → 27, two primitives → four. New since May: Mermaid and draw.io **importers** (`import-mermaid` / `import-drawio` plus Python extractors) that transcode foreign diagram source *into* the editorial style rather than rendering it natively, an SVG/PNG `export-diagram` command, a **semantic-pattern layer** (`references/semantic-patterns.md` — seven behavioural patterns that route *to* the existing visual types instead of expanding the taxonomy, the same "don't grow the catalog to grow expressiveness" instinct behind muriel's epistemic-precondition gate), a static-first accessible-motion contract (`references/animation.md`), a vendored icon system, five ADRs, and CI verifiers for label geometry, the accessible-SVG contract, and motion. The three files muriel adapted have **no commits since 2026-05-31**, so the port is still faithful; `style-guide.md`'s only changes are additive sections muriel didn't adopt (a five-token desaturated series palette for radar, a `terminal-*` CLI skin). Harvest decisions queued in [`TODO.md`](TODO.md).
