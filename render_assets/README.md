...
Importable Python assets for the [Render](../render.md) skill. Two matplotlibrc blocks and a statistical reporting helper module, extracted from [`channels/science.md`](../channels/science.md) so notebooks can `import` them instead of copy-pasting.

## Install

No dependencies beyond matplotlib. Add the render repo to your `PYTHONPATH`:

```bash
export PYTHONPATH=~/Documents/dev/render:$PYTHONPATH
```

Or `pip install -e` it:

```bash
pip install -e ~/Documents/dev/render
```

Then `from render_assets import ...` works from any Python environment.

## matplotlibrc_dark — OLED palette

Cream on near-black. Matches Render's universal OLED rule. Default for blog posts on dark sites, Scrutinizer/Psychodeli figures, paper figures destined for a dark slide deck.

```python
from render_assets import matplotlibrc_dark   # auto-applies on import
...
```python
from render_assets import matplotlibrc_light
```

For journal submissions requiring pure-white backgrounds, override the cream values:

```python
from render_assets.matplotlibrc_light import PARAMS, apply
PARAMS.update({
