"""
render_assets.contrast — WCAG 2.1 contrast ratio helpers.
...
against Render's 8:1 rule (or any threshold), and auditing an SVG by
...
.. code-block:: python
    ...
    audit_svg("examples/word-fingerprints.svg")   # prints audit table

CLI:

.. code-block:: bash

    python -m render_assets.contrast path/to/file.svg
    python -m render_assets.contrast path/to/file.svg --required 4.5
    python -m render_assets.contrast path/to/file.svg --background '#ffffff'
...
- ``RENDER_8`` = 8.0 — Render's universal rule (primary default)
...
- Parses CSS inside ``<style>`` blocks via a minimal regex — it will miss
  ...
  common idioms used by ``render_assets`` / ``marginalia`` SVGs.
...
def check_text_pair(
    ...
    Default threshold is Render's 8:1 rule.
    """
...
_TEXT_HINTS = (
    ...
    "code", "mono", "stat",
    "dropcap",
    # Render / marginalia project classes that are known text roles
...
def audit_svg(
    ...
    required
        Minimum contrast ratio for text rules. Default: Render's 8.0.
    background
...
def _main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="python -m render_assets.contrast",
    ...
    parser.add_argument(
        ...
        help="Minimum contrast ratio for text rules (default: 8.0, Render's rule).",
    )
    parser.add_argument(
