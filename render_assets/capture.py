"""
render_assets.capture — Playwright viewport-sweep screenshot helper.
...
Pulls tier data from ``render_assets.dimensions`` so named tiers
...
::
    ...
    capture_responsive(
        url="https://andyed.github.io/marginalia/pandoc/examples/us-constitution.html",
...
::
    ...
    python -m render_assets.capture https://example.com --dir captures/ --slug homepage
    python -m render_assets.capture https://example.com --dark
    python -m render_assets.capture https://example.com --selector '.hero' --slug hero
...
#: ``render_assets.dimensions`` so the viewport constants have a single
#: source of truth.
...
def slug_from_url(url: str) -> str:
    ...
    >>> slug_from_url('https://andyed.github.io/marginalia/')
...
def _main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="python -m render_assets.capture",
        description=(
