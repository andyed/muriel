"""
render_assets.styleguide — brand style guide schema, loader, and token tools.
...
render operation. Lets a Render task pick up a brand's tokens without
...
::
    ...
    python -m render_assets.styleguide examples/psychodeli-brand.toml --css
    python -m render_assets.styleguide examples/psychodeli-brand.toml --contrast
...
See ``render/examples/psychodeli-brand.toml`` and
``render/examples/render-brand.toml`` for worked examples.
...
__all__ = [
    # Types
    "Ring", "Colors", "Typography", "Meta", "Assets", "Rules",
...
class StyleGuide:
    ...
    def to_matplotlibrc(self) -> dict[str, Any]:
        ...
        Mirrors the structure of ``render_assets.matplotlibrc_dark.PARAMS``
    ...
    def audit_contrast(
        ...
        ----------
        required : float
            Minimum contrast ratio. Default 8.0 (Render's universal rule).
...
def _main(argv: Optional[list[str]] = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="python -m render_assets.styleguide",
        description=(
    ...
    parser.add_argument(
        ...
        help="Contrast threshold for --contrast. Default: 8.0 (Render's rule)",
    )
