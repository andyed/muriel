...
- [x] **Colorblind-safe palette commitment** — Wong, IBM, Tol shipped as importable constants in [`muriel/palettes.py`](../../../../muriel/palettes.py) (plus theme tier: Catppuccin, Nord). `generate_for_floor()` builds a palette at a contrast floor by construction. **CVD validation added** ([`muriel/cvd.py`](../../../../muriel/cvd.py)): Machado-2009 dichromacy simulation + CIE76 ΔE separation, composed into `muriel.palettes.validate()` and exposed as `muriel cvd` / `muriel palettes --validate`. This is the check `muriel.contrast` structurally can't make — contrast measures a color against the background; `validate` measures the palette against itself. Cross-referenced from [`channels/science.md`](channels/science.md) and [`channels/charts.md`](channels/charts.md) color sections. Numbers cross-validated against the bundled `dataviz` skill's validator (Wong adjacent: normal ΔE 59.5, deutan 48.9, tritan 16.1 — matched). Deliberate divergence: muriel runs **no** lightness-band or chroma gate (both reject muriel's own OLED register + gray-first house style when ported to muriel's surfaces).
