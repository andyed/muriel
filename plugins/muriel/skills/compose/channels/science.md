...
- [Apple Human Interface Guidelines — *Charting data*](https://developer.apple.com/design/human-interface-guidelines/charting-data) (© Apple Inc.; cite by link + paraphrase, do not copy verbatim). Apple's chart-design guidance for iOS / iPadOS / macOS surfaces — clarity over completeness, hierarchy via size + weight before colour, hue is semantic (not decorative), tap/hover detail-on-demand for dense data, Dynamic Type, light/dark adaptation. Distinctive contribution: **Swift Charts' audio-graph accessibility** — sonified data narration for VoiceOver, an accessibility surface most other platforms don't have. Read when a paper figure is also bound for an iOS app or when an accessibility audit needs a precedent. Cross-platform survey in [`vocabularies/data-viz-platforms.md`](../vocabularies/data-viz-platforms.md).
